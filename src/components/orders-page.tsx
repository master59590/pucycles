...
export function OrdersPage({ user, isAdmin }: { user: CustomerUser; isAdmin: boolean }) {
  ...
  return <CustomerShell user={user} isAdmin={isAdmin}><main className="shop-page"><div className="shop-page-heading"><span className="section-kicker">{t.kicker}</span><h1>{t.title}</h1><p>{t.intro}</p></div>{shopError && <p className="field-error" role="alert">{shopError}</p>}
    {!hydrated ? <div className="shop-loading">{t.loading}</div> : orders.length === 0 ? <div className="shop-empty"><PackageSearch /><h2>{t.emptyTitle}</h2><p>{t.emptyText}</p><Link className="shop-primary-button" href="/products">{t.browse}</Link></div> : <div className="customer-orders">{orders.map((order) => { const total = order.subtotalThb + (order.shippingFeeThb ?? 0); return <Link href={`/orders/${order.orderNumber}`} key={order.orderNumber}><div><span className={`order-status ${order.status}`}>{orderStatusCopy[order.status][preferences.locale]}</span><h2>{order.orderNumber}</h2><p>{new Date(order.createdAt).toLocaleDateString(preferences.locale === "th" ? "th-TH" : "en-GB", { dateStyle: "medium" })} · {order.lines.reduce((sum, line) => sum + line.quantity, 0)} {t.items}</p></div><div><strong>{order.shippingFeeThb === null ? t.shippingPending : formatMoney(total, order.address.countryCode, preferences.locale, order.exchangeRate)}</strong><ArrowUpRight /></div></Link>; })}</div>}
  </main></CustomerShell>;
}
