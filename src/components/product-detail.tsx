...
export function ProductDetail({ product, user, isAdmin }: { product: Product; user: CustomerUser | null; isAdmin: boolean }) {
  const { preferences, addToCart, exchangeRates } = useShop();
  ...
  return <CustomerShell user={user} isAdmin={isAdmin}><main className="detail-page">
    ...
    <section className="product-detail-layout">
      ...
      <div className="detail-copy"><span className={`stock-status ${product.stockStatus}`}><span />{product.stockStatus.replaceAll("_", " ")}</span><p className="product-brand">{product.brand} · {product.sku}</p><h1>{name}</h1><p className="detail-description">{description}</p><strong className="detail-price">{formatMoney(product.priceThb, preferences.countryCode, preferences.locale, exchangeRates[country.currency])}</strong>
        <div className="detail-specs"><div><Wrench /><span>Exact fitment<strong>{(product.fitments?.length ? product.fitments.map((fitment) => `${fitment.model} (${fitment.yearFrom}-${fitment.yearTo})`) : [`${product.models.join(" / ")} (${product.yearFrom}-${product.yearTo})`]).join(" · ")}</strong></span></div><div><Check /><span>Compatibility<strong>Confirm your motorcycle model and year before ordering</strong></span></div><div><Scale /><span>Weight<strong>{(product.weightGrams / 1000).toFixed(2)} kg</strong></span></div></div>
